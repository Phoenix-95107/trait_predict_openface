...
class FacialAnalyzer:
    ...
    def get_head_pose_openface(self, openface_data):
        ...
        try:
            ...
            if ' AU25_r' in openface_data.keys() and ' AU26_r' in openface_data.keys():
...
def calculate_section2(openface_results):
    ...
    for i, result_data in df.iterrows():
        try:
            ...
            persistant = (1 - abs(result['jaw_angle']-100)/40)*0.3 + (1 - abs(result['head_pitch'])/30)*0.4 + (au_values.get('AU01', 0) + au_values.get('AU06', 0))/2*0.3
            persistant_list.append(persistant)
            
            out_focus = (1 - abs(result['head_pitch'])/30)*0.4 + (au_values.get('AU01', 0) + au_values.get('AU02', 0) + au_values.get('AU06', 0))*0.2
            ...
            out_risk = (1 if result['head_pitch'] > 5 else 0) *0.4 + (1 if result['jaw_angle']>120 else 0)*0.3 + au_values.get('AU04', 0) * 0.3
