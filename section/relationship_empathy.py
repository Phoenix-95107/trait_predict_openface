...
class FacialAnalyzer:
    ...
    def process_openface_data(self, openface_data):
        ...
        try:
            ...
            gaze_direction, iris_ratio = self.calculate_gaze_iris_openface(openface_data)
            return {
    ...
    def calculate_gaze_iris_openface(self, openface_data):
        ...
        try:
            ...
            if all(col in openface_data.keys() for col in [' gaze_0_x', ' gaze_0_y', ' gaze_1_x', ' gaze_1_y']):
            ...
            if ' AU45_r' in openface_data.index:
                ...
                eye_openness = 1.0 - min(blink_value / 5.0, 1.0)  # OpenFace AU intensities are typically in 0-5 range
...
def calculate_section1(openface_results):
        ...
        try:
            ...
            for au in ['AU01', 'AU06', 'AU12']:
                ...
                if au_col in data.keys():
                    au_values[au] = data[au_col].mean() / 5.0  # Normalize to 0-1 range
                else:
            ...
            trust.append((au_values.get('AU06', 0) + au_values.get('AU12', 0))*0.2 + 
                         result['gaze_direction']*0.4 + iris_score*0.2)
            
            openness.append(au_values.get('AU12', 0)*0.5 + 
                           au_values.get('AU06', 0)*0.3 + 
                           result['gaze_direction']*0.2)
            
            Empathy.append(au_values.get('AU01', 0)*0.5 + 
                          au_values.get('AU06', 0)*0.3 + 
                          result['gaze_direction']*0.2)
            
            ConflictAvoid.append((1-au_values.get('AU12', 0))*0.5 + 
    ...
    try:
        ...
        return {
            "trust": {"balance": f"{np.mean(trust)*100:.1f}%", "top_image": int(select[0])},
