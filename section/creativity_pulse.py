    ...
    def calculate_facial_symmetry(self, row):
        ...
        try:
            ...
            for i in range(len(x_cols) // 2):
                ...
                dist_diff = abs(
                    abs(left_x - midline_x) - abs(right_x - midline_x))
            ...
            symmetry_score = max(0, 1 -
                                 (avg_asymmetry * 10))  # Scaled for simplicity
...
def calculate_section3(openface_results):
    ...
    for i, result_data in df.iterrows():
        try:
            ...
            micro_expr_score = 1.0 if any([
                au_values.get(au, 0) > 0.9
                for au in ['AU01', 'AU02', 'AU04', 'AU12']
            ]) else 0.0

            # Calculate metrics using the same formulas as before
            ideation = (
                au_values.get('AU12', 0) + au_values.get('AU06', 0)
            ) * 0.2 + metrics['eye_openness'] * 0.3 + micro_expr_score * 0.3
            ideation_list.append(make_score(ideation))

            openness = (au_values.get('AU02', 0) + au_values.get(
                'AU05', 0)) * 0.3 + metrics['facial_symmetry'] * 0.4
            openness_list.append(make_score(openness))

            originalty = (1 -
                          metrics['facial_symmetry']) * 0.4 + au_values.get(
                              'AU12', 0) * 0.3 + au_values.get('AU01', 0) * 0.3
            originalty_list.append(make_score(originalty))

            attention = metrics['eye_openness'] * 0.3 + (
                1 - metrics['facial_symmetry']) * 0.3 + au_values.get(
                    'AU04', 0) * 0.4
            attention_list.append(make_score(attention))
        except Exception as e:
    ...
    try:
        ...
        select = np.argmax(
            [ideation_list, openness_list, originalty_list, attention_list],
            axis=1)

        return {
            'ideation': {
                "balance": f"{np.mean(ideation_list)*100:.1f}%",
                "top_image": int(select[0])
            },
            'openness': {
                "balance": f"{np.mean(openness_list)*100:.1f}%",
                "top_image": int(select[1])
            },
            'originalty': {
                "balance": f"{np.mean(originalty_list)*100:.1f}%",
                "top_image": int(select[2])
            },
            'attention': {
                "balance": f"{np.mean(attention_list)*100:.1f}%",
